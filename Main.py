    ...
    def save_and_close():
        ...
        try:
            with open(CONFIG_FILE, 'w') as f:
                json.dump({'CLIENT_ID': c_id, 'CLIENT_SECRET': c_sec, 'STREAMER_LOGIN': streamer}, f)
            setup_root.destroy()
        except Exception as e:
            messagebox.showerror("Error", f"Could not save file: {e}")
    tk.Button(setup_root, text="Save & Continue", bg='#a970ff', fg='white', font=('Segoe UI', 10, 'bold'), activebackground='#9146ff', activeforeground='white', bd=0, cursor="hand2", command=save_and_close).pack(fill="x", padx=40, ipady=5)
    setup_root.mainloop()
...
class TwitchDashboard:
    ...
    def save_stats(self):
        try:
            with open(DATA_FILE, 'w') as f:
                json.dump({'watch_seconds': self.total_watch_seconds, 'live_sessions': self.live_session_count, 'viewers_sum': self.viewers_sum, 'viewers_samples': self.viewers_samples, 'viewers_min': self.viewers_min, 'viewers_max': self.viewers_max}, f)
        except: pass
    ...
    def background_monitor(self):
        ...
        while True:
            if api_countdown <= 0:
                try:
                    if not self.token: self.token = self.get_twitch_token()
            ...
            if self.is_live:
                ...
                if self.total_watch_seconds % 60 == 0: self.save_stats()
            time.sleep(1)
            api_countdown -= 1
